...
struct EmojiMemoryGameView: View {
    ...
    // private state used to temporary track whether
    // a card has been dealt or not
    ...
    // in the ViewModel's (and the Model's) cards array
    ...
    // returns a Double that gets bigger the closer a card is to the front of the cards array
    // used by both the matchedGeometryEffect CardViews so that
    // they order the cards in the "z" direction the same way
    ...
    // (does not include any of the control buttons or the deck)
        ...
        AspectVGrid(items: game.cards, aspectRatio: 2/3) { card in
            if isUndealt(card) || (card.isMatched && !card.isFaceUp) {
                ...
                CardView(card: card)
                    ...
                    // if that one arrives/departs the UI at the same time
                    // as departing/arriving, then the arriving one will fly across the screen
                    // (and resize) from where the departing one left
    ...
    // the body of the deck from which the cards are dealt
    var deckBody: some View {
        ZStack {
            ForEach(game.cards.filter(isUndealt)) { card in
                CardView(card: card)
                    ...
                    // don't want it to ALSO fade or scale out (since transitions and matchedGeometryEffects are not mutually exclusive)
        ...
        .onTapGesture {
            ...
            // (instead it is just setting some of the private @State)
    ...
    var shuffle: some View {
        Button("Shuffle") {
            ...
            /* add this intent func to ViewModel */
            withAnimation {
                game.shuffle()
            }
        }
    ...
    var restart: some View {
        Button("Restart") {
            // animated user Intent function call
            // at the same time, resetting the local "dealing" private State
            /* add this intent func to ViewModel */
            withAnimation {
                dealt = []
                game.restart()
...
struct CardView: View {
    ...
    var body: some View {
        GeometryReader { geometry in
            ZStack {
                // group is a "bag of Legos" container
                // useful for propagating view modifiers to multiple views
                ...
                Group {
                    ...
                    // any time it's not consuming bonus time, the lower Pie appears
                    // (not animated, just showing how much time is left).
                    // any time it is consuming bonus time, the upper Pie appears.
                    // when it appears (onAppear), it starts animating its own endAngle
                ...
                Text(card.content)
                    ...
                    .animation(.linear(duration: 0.65).repeatForever(autoreverses: false), value: card.isMatched)
                    .padding(5)
            ...
            // it turns the ZStack with a Pie and Text in it into a "card" on screen
            // it does this by returning its own ZStack with RoundedRectangles in it
            // see Cardify.swift
            .cardify(isFaceUp: card.isFaceUp)
        }
    }
    
    // "scale factor" to scale the Text up so that it fits the geometry.size offered
